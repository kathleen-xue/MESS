...
//RECONSTRUCTING PATH
	long long minimum = 0; //constructing path of largest negative cost (backtracking), minimum denotes current largest negative cost
...
//INITIALIZE DEMAND MATRIX
	for(int trials = 1; trials <= numTrials; trials++) {
		...
		vector<vector<int> > demands = createDemands(M, T);
		...
		for(int z = lowerBound; z <= upperBound; z += increments) {
			...
			for(int k = 0; k < numBatteries; k++) { //loops through the batteries
				...
				path P;
				cheapestPathDP(T, M, P, transportCost, dpCUBE, demandsMicro); //actually run algorithm

				//PRINT PATH
				long long totalnegs = 0;
				long long totalben = 0;
			...
			cout << "MESS TOTAL NET BENEFIT: " << DPtotalBen << endl;
			cout << "GREEDY TOTAL NET BENEFIT: " << GreedytotalBen << endl;
			cout << endl;
