...
vector<vector<vector<long double> > > cheapestPathDP(int T, int M, vector<pair<pair<int, int>, int> >& path) {
...
int main () {
	...
	vector<vector<vector<long double> > > dpCUBE = cheapestPathDP(T, M, path);
	cout << "Minimum cost path for current battery:" << endl;
